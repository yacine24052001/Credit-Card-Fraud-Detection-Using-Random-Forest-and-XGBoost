...
## validation croisée
```{r}
fraudnev<- vfold_cv(train_data, v= 3, strata = isFraud)
fraudnev
```
## applying receipies on the data
# Create a recipe
```{r}
recipe_obj <- recipe(isFraud ~ ., data = train_data) %>%
  ...
  step_smote(isFraud, over_ratio = 1, neighbors = 100)
```
...
```{r}
Fraudworkflow <- workflow() %>%
  add_model(rf_model) %>%
  add_recipe(recipe_obj)
Fraudworkflow
```
## Création du calcule paral
```{r}
...
## graphe
```{r}
ggplot(Train_data1, aes(x = isFraud)) +
  geom_bar() +
  labs(title = "Distribution of isFraud in Training Data After SMOTE", x = "isFraud", y = "Count")
```
...
#pr_auc (precision recall auc) tidy models)
